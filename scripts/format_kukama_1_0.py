...
def reformat_file(old_path, new_path):
    ...
    TITLES = {
        ...
        "Morpheme Gloss(English)": "Morpheme Gloss(English):".ljust(TITLE_WIDTH) + "\t",
        "Morpheme Gloss(Spanish)": "Morpheme Gloss(Spanish):".ljust(TITLE_WIDTH) + "\t",
    ...
    for block in blocks:
        ...
        for ln in block:
            ...
            if len(parts) >= 2:
                ...
                # Normal tag handling
                    ...
                    mb_line = content
                elif tag == "Morpheme" and content.startswith("Gloss(English)"):
                    ge_line = content[14:].lstrip()
                elif tag == "Morpheme" and content.startswith("Gloss(Spanish)"):
                    ps_line = content[14:].lstrip()
        ...
        if tx_line:
            ...
            if tr_text_eng and len(tr_text_eng.strip()) > 1: reformatted.append(TITLES["English Sent Gloss:"] + tr_text_eng)
            if tr_text_spa and len(tr_text_spa.strip()) > 1: reformatted.append(TITLES["Spanish Sent Gloss"] + tr_text_spa)
