...
def format_umr_writer_document(input_file, replace_json):
    input_file = str(input_file)

    with open(input_file, "r") as f:
        text = f.read()

    # Remove metadata block at the beginning
    text = re.sub(r"^(user name:.*?\nexport time:.*?\n)", "", text, flags=re.DOTALL)
    # Remove everything after # Source File:
    text = re.sub(r"# Source File:.*", "", text, flags=re.DOTALL)
    # Replace `# ::snt\d Sentence:` with `# ::snt`
    if "tlp_umr_wa_1120" in input_file:
        text = re.sub(r"# ::snt\d+ Sentence:", "# ::snt", text)
    else:
        text = re.sub(r"# :: snt\d+	", "# ::snt", text)

    # Load the JSON file
    with open(replace_json, 'r') as file:
...
def format_chinese():
    ...
    for file_path in input_folder_path.iterdir():
        if file_path.suffix == '.txt':  # Ensure the file has a .txt extension
            try:
                output_list = format_umr_writer_document(file_path, Path(root) / 'chinese/role_mappings.json')
                with open(Path.joinpath(output_folder_path, file_path.name), 'w') as outfile:
                    outfile.writelines(output_list)
            except Exception as e:
                print(f"Error processing {file_path}: {e}")
